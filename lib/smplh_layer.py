...
class SMPLH_Layer(Module):
    ...
    def __init__(self,
        ...
        self.gender = gender
        self.hands = hands
        assert self.hands, 'must use hands for SMPLH model'
        if gender == 'neutral':
            self.model_path = join(model_root, 'neutral_model.pkl')
        elif gender == 'female':
            self.model_path = join(model_root, 'female_model.pkl')
        elif gender == 'male':
            self.model_path = join(model_root, 'male_model.pkl')
        ...
        # jr = np.array(smpl_data['J_regressor'].toarray()) if 'float' not in smpl_data['J_regressor'].dtype else smpl_data['J_regressor']
        self.register_buffer(
            'th_J_regressor',
    ...
    def forward(self,
        ...
        th_T = torch.matmul(th_results2, self.th_weights.transpose(0, 1)) # pose blend shape, convert joints pose to verts offsets?
