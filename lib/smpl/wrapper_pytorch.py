...
class SMPLPyTorchWrapperBatchSplitParams(nn.Module):
    ...
    Alternate implementation of SMPLPyTorchWrapperBatch that allows us to independently optimise:
     ...
     3. top betas (primarily adjusts bone lengths)
     4. other betas
    """
    def __init__(self, model_root, batch_sz, top_betas=None, other_betas=None, global_pose=None, other_pose=None,
                 trans=None, offsets=None, faces=None, gender='male', num_betas=300):
        ...
        if other_betas is None:
            self.other_betas = nn.Parameter(torch.zeros(batch_sz, num_betas-2))
        else:
